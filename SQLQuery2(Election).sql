...
SELECT COUNT(*) AS total_rows,
       COUNT(DISTINCT parliament_constituency) AS unique_constituencies
FROM constituencywise_results;

SELECT parliament_constituency, COUNT(*) AS TOTAL_CANDIDATES
FROM constituencywise_results
GROUP BY parliament_constituency;
...
SELECT TOP 1
    p.party_alliance,
    COUNT(cr.Constituency_ID) AS Most_Seats_Won
FROM 
    constituencywise_results cr
JOIN 
    partywise_results p ON cr.Party_ID = p.Party_ID
...
JOIN 
    partywise_results p ON cr.Party_ID = p.Party_ID
WHERE 
    s.State = 'Karnataka' GROUP BY s.State;
